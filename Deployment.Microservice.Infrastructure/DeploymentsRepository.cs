...
namespace Deployment.Microservice.Infrastructure
{
    public class DeploymentsRepository : IDeploymentsRepository
    {
        ...
        private readonly IGCPInfrastructureRepository _cpRepository;
        private readonly ICustomerPipelinesRepository _pipelines;

        public DeploymentsRepository(DeploymentsDBContext dbContext, IPipelinesRepository pipe, IGCPInfrastructureRepository gc, ICustomerPipelinesRepository pipelines)
        {
        ...
        {
            var result = _dbContext.DeploymentsDomain.Where(a=>a.CUSTOMER_ID == id).ToList();
                    ...
                    FetchOptions =
            {
                ...
                using (var repository = new Repository(localPath))
                {
                    ...
                    var fileContentResult1 = await _cpRepository.CreateNewInfrastructure(
                        CUSTOMER_ID, language, "main-template", 18, REGION, CLUSTER_NAME,
                    ...
                    var fileContentResult2 = await _cpRepository.CreateNewInfrastructure(
                        CUSTOMER_ID, language, "variables-template", 16, REGION, CLUSTER_NAME,
                        ARTIFACT_REGISTRY_REGION, ARTIFACT_REGISTRY, appname
                    );
  ...
  ""universe_domain"": ""{jsonData?.universe_domain}""
                ...
                _dbContext.DeploymentsDomain.Add(repo);
